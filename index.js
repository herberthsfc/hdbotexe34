		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '*「 💎 」 Comando disponível apenas para o Herberth!*',
...
case 'lofi':
					...
					break
					case 'oi':
          if (args.length < 1) return reply('olá, como voce está?')
          break
          case 'siri':
					hobby = body.slice(1)
					const hob =['Olá rsrs','Sim, Eu acho','Não sei','Por quê?','Sei lá','Eu não quero falar sobre isso','Foda-se','Voce é uma gostosa','kkkkkk','Não fala comigo','Virgem','Pesquisa no google','Lixo','Meu pau','Cala a boca','Eu acho que voce tentou ser engraçado(a)','Eu quero comer uma bolacha','Fdp']
					...
					client.sendMessage(from, 'Nome : *'+ganteng+'*\n\nPorcentagem gay : '+ teng+'%', text, { quoted: mek })
					break
          case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Musica Encontrada!!!*\nTitulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE. ENVIANDO! POR FAVOR, NAO ENVIE SPAM *`
			...
			case 'mapa': 
				...
				case 'adicionar':
					...
					if (args[0] === 'abrir') {
						...
						reply(`*「 ❗ 」 Grupo fechado parar membros comuns!*`)
       ...
       client.sendMessage(from, '*「 💎 」 Este é o contato do meu criador, por favor, salve!*',MessageType.text, { quoted: mek} )
          ...
          if (args.length < 1) return reply('*「 ❗ 」 Onde está o texto?* \ nExemplo: ${prefix}pubglogo herberth|lindo')
          if (!isUser) return reply(mess.only.daftarB)
          ...
          buffer = await getBuffer(anu.result)
          client.sendMessage(from, buffer, image, {quoted: mek, caption: '*「 ✔️ 」 Aqui está o seu logotipo!*'})
          break
				case 'wait':
				case 'pesquisar':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
